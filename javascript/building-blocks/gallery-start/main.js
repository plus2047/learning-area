const displayedImage = document.querySelector('.displayed-img');
const thumbBar = document.querySelector('.thumb-bar');

const btn = document.querySelector('button');
const overlay = document.querySelector('.overlay');

/* Declaring the array of image filenames */
const imageArray = ["images/pic1.jpg", "images/pic2.jpg", "images/pic3.jpg", "images/pic4.jpg", "images/pic5.jpg"]

/* Declaring the alternative text for each image file */
const imageAlt = ["Eye", "Ground", "Flower", "Mural", "Butterfly"]

/* Looping through images */
for(let i = 0; i < imageArray.length && i < imageAlt.length; i++) {
    const newImage = document.createElement('img');
    newImage.setAttribute('src', imageArray[i]);
    newImage.setAttribute('alt', imageAlt[i]);
    newImage.addEventListener("click", () => {
        displayedImage.setAttribute("src", imageArray[i]);
        displayedImage.setAttribute("alt", imageAlt[i]);
    })
    thumbBar.appendChild(newImage);
}

/* Wiring up the Darken/Lighten button */
btn.addEventListener("click", () => {
    const darkenColor = "rgba(0,0,0,0.2)";
    const lightenColor = "rgba(0,0,0,0)";
    if(btn.getAttribute("class") === "dark") {
        overlay.style.backgroundColor = darkenColor;
        btn.setAttribute("class", "light");
        btn.textContent = "Lighten"
    } else {
        overlay.style.backgroundColor = lightenColor;
        btn.setAttribute("class", "dark");
        btn.textContent = "Darken"
    }
})
